...
Supabase provides the database and server functions used by the guestbook and other site features. Cloudflare Turnstile provides bot verification on the entry screen. Cloudflare processes technical signals such as a network address, browser headers, and the site origin to detect bots and improve its detection service. The guestbook server verifies the challenge token with Cloudflare and issues a signed posting session that expires after one hour. This session is kept in app memory, contains a keyed network hash for posting limits, and is checked against the site origin when you post. Network changes do not reset that session's posting limits or require another bot check. The session is not saved in browser storage or the guestbook database. Links to these providers' privacy notices appear below.
