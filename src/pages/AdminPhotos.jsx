...
const emptyPhoto = { id: '', title: '', album_id: '', image_url: '', caption: '', sort_order: 0, width: '', height: '', is_published: true };
...
function AdminPhotos({ secret, library, onChange, onBusy }) {
  ...
  function selectPhoto(item) {
    ...
    setPhoto(item ? { ...emptyPhoto, ...item, album_id: item.album_id || '', width: item.width || '', height: item.height || '' } : {
  ...
  async function savePhoto(event) {
    ...
    try {
      ...
      setPhoto({ ...emptyPhoto, ...result.photo, album_id: result.photo.album_id || '', width: result.photo.width || '', height: result.photo.height || '' });
      setFile(null);
      setDirty(false);
      setStatus('saved');
  ...
  const matching = library.photos.filter((item) => `${item.title} ${item.caption}`.toLowerCase().includes(query.toLowerCase()) &&
    (filter === 'all' || (filter === 'published' ? item.is_published : !item.is_published)));
  const albumPhotos = album?.id ? library.photos.filter((item) => item.album_id === album.id) : [];
    ...
    <main className="admin-page admin-page--mission-control admin-photos">
      ...
      {mode === 'photos' ? (
          ...
          {photo ? (
            <form className="admin-photos__editor" onSubmit={savePhoto}>
              <fieldset disabled={working}>
                ...
                <button type="submit">{status === 'saving' ? 'Saving photo...' : 'Save photo'}</button>
              </fieldset>
            </form>
        ...
        <div className="admin-photos__layout">
          ...
          {album ? <form className="admin-photos__editor" onSubmit={saveAlbum}>
            <fieldset disabled={working}>
              <legend>{album.id ? 'Edit album' : 'New album'}</legend>
              <label>Album title<input required maxLength={120} value={album.title} onChange={(event) => { setAlbum({ ...album, title: event.target.value }); setAlbumDirty(true); }} /></label>
              <label>Description<textarea maxLength={1000} value={album.description} onChange={(event) => { setAlbum({ ...album, description: event.target.value }); setAlbumDirty(true); }} /></label>
              <label>Display order<input type="number" min="0" max="1000000" step="1" required value={album.sort_order} onChange={(event) => { setAlbum({ ...album, sort_order: event.target.value }); setAlbumDirty(true); }} /></label>
              <p className="admin-page__hint">Lower numbers appear first. Add published photos to make this album visible.</p>
          ...
          </form> : <div className="admin-photos__placeholder"><h2>Put a collection together.</h2><p>Create an album, then assign photos to it.</p></div>}
        </div>
      )}
      </div>
    </main>
